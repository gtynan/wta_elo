...
from src.data_scraping import get_raw_games
...
def test_get_raw_games():
    ...
    data = get_raw_games(filename, year_from, year_to, save=True).sort_values(by=t_date_col)

    # this is the pipeline file so should already have been saved
    assert path.exists(f'data/01_raw/{filename}_from_{year_from}_to_{year_to}.csv')
